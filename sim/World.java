...
/**
 * The top level game class that is used by other packages to interact with the Symbibots world model.
 * @author  Matt Oates
 */
public class World implements Runnable, Visualisable {
	...
	private int expressTickInterval = 400;
	...
	public void run() {
		while (!destroy) {
			PAUSED:if (! ui.isPaused()) {
				
				//If we are doing a data run only play to a limited epoch
				if (IS_DATA_RUN) {
					if (epochs > GeneticAlgorithm.DATA_RUN_EPOCH_LIMIT) {
						ui.setPaused(true);
						ui.addLog("Data run completed...");
						JOptionPane.showMessageDialog(ui, "Data run complete, you can now save the stats file.");
						//Reset random number generator
						GeneticAlgorithm.DATA_RUN_RANDOM.setSeed(8589934592l);
						break PAUSED;
					}
				}
		...
		ui.addLog("Game world destroyed!");
	}
